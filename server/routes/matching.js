...
function runMatching(actorId) {
  ...
  const alreadyMatched = new Set(
    db.prepare(`SELECT listing_id FROM matches WHERE status NOT IN ('cancelled')`).all().map((r) => r.listing_id)
  );
  const openNeeds = new Set(
    db.prepare(`SELECT need_id FROM matches WHERE status NOT IN ('cancelled')`).all().map((r) => r.need_id)
  );
  ...
  for (const m of selected) {
    ...
    db.prepare(`UPDATE food_listings SET status = 'matched' WHERE id = ?`).run(m.listing_id);

    const listing = db.prepare('SELECT * FROM food_listings WHERE id = ?').get(m.listing_id);
    const need = db.prepare('SELECT * FROM food_needs WHERE id = ?').get(m.need_id);
    ...
    audit(actorId, 'AUTO_MATCH', 'matches', ins.lastInsertRowid, {
      score: m.score,
    ...
    created.push(ins.lastInsertRowid);
  }

  const durationMs = Number(process.hrtime.bigint() - startedAt) / 1e6;
  audit(actorId, 'RUN_MATCHING', 'system', null, {
...
router.post('/run', requireRole('admin'), requireActive, (req, res) => {
  const result = runMatching(req.user.id);
  res.json({ message: `Matching selesai: ${result.created} match dibuat`, ...result });
});

router.get('/', requireRole('admin', 'courier', 'donor', 'recipient'), (req, res) => {
  let rows;
